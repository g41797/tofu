...
pub const Stage0Wake = struct {
    ...
    pub fn init() !Stage0Wake {
        ...
        const status = ntdll.NtCreateIoCompletion(
            &h,
    ...
    pub fn runLoop(self: *Stage0Wake) !void {
        var entries: [1]ntdll.FILE_COMPLETION_INFORMATION_EX = undefined;
        var removed: u32 = 0;

        while (true) {
            // NtRemoveIoCompletionEx(HANDLE, PFILE_COMPLETION_INFORMATION_EX, ULONG, PULONG, PLARGE_INTEGER, BOOLEAN)
            const status = ntdll.NtRemoveIoCompletionEx(
                self.iocp,
                &entries,
                1,
                &removed,
            ...
            if (removed > 0) {
                const key = entries[0].CompletionKey;
                if (key == SHUTDOWN_KEY) {
